...
namespace ReportesTranspesa.Formularios.Areas.Operaciones.ProgramacionViajes
{
    ...
    class Ticket
    {
        ...
        public int FontCodigo
        {
            get { return fontCodigo; }
            set { if (value != fontCodigo) fontCodigo = value; }
        }
        ...
        public void PrintTicket(string impresora = "")
        {
            ...
            printFontCodigo = new Font(fontName, fontCodigo, FontStyle.Regular);
            PrintDocument pr = new PrintDocument();
            pr.PrinterSettings.Copies = copias;
            if (!impresora.Equals(""))
            {
        ...
        private void DrawImage()
        {
            if (headerImage != null)
            {
                try
                {
                    ...
                    gfx.DrawImage(headerImage, new Point((int)3, (int)YPosition()));      //AJUSTAR POSICION DE IMAGEN
        ...
        {
            foreach (string subHeader in subHeaderLines2)
            {
                if (subHeader.Length > maxChar)
                {
                    int currentChar = 0;
                    ...
                    {
                        line = subHeader;
                        gfx.DrawString(line.Substring(currentChar, maxChar), printFontCodigo, myBrush, leftMargin, YPosition(), new StringFormat());
                    ...
                    line = subHeader;
                    gfx.DrawString(line.Substring(currentChar, line.Length - currentChar), printFontCodigo, myBrush, leftMargin, YPosition(), new StringFormat());
                    count++;
                }
                else
                {
                    ...
                    gfx.DrawString(line, printFontCodigo, myBrush, leftMargin, YPosition(), new StringFormat());
