...
-- CREAR TABLA ReportesApp_Operaciones_Operatividad_Condiciones Y LLENARLA

-- CREAR TABLA ReportesApp_Operaciones_Operatividad_View

-- CREAR TABLA ReportesApp_Operaciones_Operatividad_PlacasView

-- CREAR TABLA ReportesApp_Operaciones_Operatividad_Registro

-- CREAR TABLA ReportesApp_Operaciones_Operatividad_Permisos Y LLENARLA
...
-- Description:	LISTAR TRACTOS
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Operatividad_ListarTractos]
@Opcion INT,
@Periodo VARCHAR(6)
AS
BEGIN
	IF (@Opcion = 1) BEGIN		-- LISTAR TRACTOS
		SELECT UC.IdUnidad, TV.Descripcion AS 'TIPO_UNIDAD', V.NumeroPlaca 'PLACA', SV.Descripcion AS 'SUBTIPO_UNIDAD',
		CASE WHEN O.IdOperacion = 5 THEN 'SIN OPERACION' ELSE O.Descripcion END AS 'PROGRAMACION'
		...
		LEFT JOIN ReportesApp_Operaciones_Operatividad_PlacasView P ON P.IdUnidad = UC.IdUnidad
																	AND P.Anio = RIGHT(@Periodo,4)
																	AND P.Mes = LEFT(@Periodo,2)
		WHERE (TV.idTipoVehiculo = 1) AND (V.Estado = 2) AND P.IdUnidad IS NULL
		UNION
		SELECT UC.IdUnidad, TV.Descripcion AS 'TIPO_UNIDAD', V.NumeroPlaca 'PLACA', SV.Descripcion AS 'SUBTIPO_UNIDAD',
		CASE WHEN O.IdOperacion = 5 THEN 'SIN OPERACION' ELSE O.Descripcion END AS 'PROGRAMACION'
		FROM ReportesApp_Operacion_MaestroUnidadesConductor UC
		...
		LEFT JOIN OP_TR_SubTipoVehiculo SV ON SV.SubTipoVehiculo = V.SubTipoVehiculo
		LEFT JOIN ReportesApp_Operacion_Previaje_Operaciones O ON O.IdOperacion = UC.IdProgramacion
		LEFT JOIN ReportesApp_Operaciones_Operatividad_CarretasView P ON P.IdUnidad = UC.IdUnidad
																	AND P.Anio = RIGHT(@Periodo,4)
																	AND P.Mes = LEFT(@Periodo,2)
		WHERE (TV.idTipoVehiculo = 2) AND (V.Estado = 2) AND P.IdUnidad IS NULL
		ORDER BY V.NumeroPlaca
	END
	...
	IF (@Opcion = 2) BEGIN		-- LISTAR MAPEADOS
		...
		LEFT JOIN ReportesApp_Operacion_MaestroUnidadesConductor UC ON UC.IdUnidad = P.IdUnidad
		LEFT JOIN OP_TR_Vehiculo V ON V.IdVehiculo = UC.IdUnidad
		LEFT JOIN ReportesApp_Operaciones_ConductorUnidades_TipoVehiculo AS TV WITH(NOLOCK) ON TV.idTipoVehiculo = V.TIPOVEHICULO
		LEFT JOIN OP_TR_SubTipoVehiculo SV ON SV.SubTipoVehiculo = V.SubTipoVehiculo
		LEFT JOIN ReportesApp_Operacion_Previaje_Operaciones O ON O.IdOperacion = UC.IdProgramacion
	...
	IF (@Opcion = 3) BEGIN		-- LISTAR CONDICIONES
		SELECT idCondicion, Condicion FROM ReportesApp_Operaciones_Operatividad_Condiciones
		ORDER BY Orden
	END

	IF (@Opcion = 4) BEGIN		-- LISTAR OPERACIONES
		SELECT * FROM ReportesApp_Operacion_Previaje_Operaciones
		WHERE IdOperacion IN (2,3,1,4,10)
	END

	IF (@Opcion = 5) BEGIN		-- LISTAR OPERACIONES PARA LAVADO
		SELECT * FROM ReportesApp_Operacion_Previaje_Operaciones
		WHERE IdOperacion NOT IN (7,8)
		UNION
		SELECT 11, 'SIN OPERACION'
	END
END
...
-- Author:		GERARDO REYES
-- Create date: 30-03-2024
-- Description:	MAPEAR CONDICIONES
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Operatividad_MapearCondiciones]
@idCondicion INT,
@Periodo VARCHAR(6),
@Usuario VARCHAR(20)
AS
...
BEGIN TRAN
BEGIN TRY
	IF (NOT EXISTS(SELECT * FROM ReportesApp_Operaciones_Operatividad_View WHERE idCondicion = @idCondicion
	AND Anio = RIGHT(@Periodo,4) AND Mes = LEFT(@Periodo,2))) BEGIN		-- CONTINUAR
		...
		IF (@n IS NULL) BEGIN
			INSERT INTO ReportesApp_Operaciones_Operatividad_View (idCondicion,Anio,Mes,NroDias)
		...
		WHILE (@n IS NOT NULL) BEGIN
			...
			IF NOT EXISTS(SELECT TOP 1 Mes FROM ReportesApp_Operaciones_Operatividad_View WITH(NOLOCK) WHERE idCondicion = @idCondicionDF 
			...
			UPDATE ReportesApp_Operaciones_Operatividad_View SET 
...
-- Description:	LISTAR TABLA DE TRACTOS
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Operatividad_ListarTablaOperatividadTractos]
@Periodo VARCHAR(6),
@Placa VARCHAR(30),
...
BEGIN
	...
	DECLARE @T_Prueba TABLE (IdUnidad INT, Programacion VARCHAR(100), Placa VARCHAR(50),
	...
	INSERT INTO @T_Prueba (Programacion,Placa)
	VALUES ('Programacion', '<PLACA>')
	...
	IF (@Programacion = 'TODO') BEGIN
		INSERT INTO @T_Prueba (IdUnidad, Programacion, Placa)
		...
		LEFT JOIN OP_TR_Vehiculo V ON V.IdVehiculo = UC.IdUnidad
		LEFT JOIN ReportesApp_Operacion_Previaje_Operaciones O ON O.IdOperacion = UC.IdProgramacion
		INNER JOIN ReportesApp_Operaciones_Operatividad_PlacasView P ON P.IdUnidad = UC.IdUnidad
	...
	WHILE @FechaVer <= @FechaFin BEGIN
		...
		ELSE T.D31 END
		FROM @T_Prueba AS T 
		LEFT JOIN ReportesApp_Operaciones_Operatividad_PlacasView AS A WITH(NOLOCK) ON T.IdUnidad = A.IdUnidad 
	...
	CREATE TABLE #PruebaView (IdUnidad INT, Programacion VARCHAR(100), Placa VARCHAR(50),
	...
	IF @CantDias = 28 BEGIN
		SET @SQL='SELECT IdUnidad, Placa AS PLACA, Programacion AS PROGRAMACION,
...
-- Author Name:	GERARDO REYES
-- Create date: 01-04-2024
-- Description:	LISTAR TABLA DE OPERATIVIDAD
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Operatividad_ListarTablaOperatividad]
@Periodo VARCHAR(6)
AS
BEGIN
	DECLARE @Anio SMALLINT = RIGHT(@periodo,4)
	...
	INNER JOIN ReportesApp_Operaciones_Operatividad_View T ON T.idCondicion = C.idCondicion 
	...
	WHILE @FechaVer <= @FechaFin BEGIN
		...
		ELSE T.D31 END
		FROM @T_Prueba AS T 
		LEFT JOIN ReportesApp_Operaciones_Operatividad_View AS A WITH(NOLOCK) ON T.idCondicion = A.idCondicion 
		WHERE A.Anio = YEAR(@FechaVer) AND A.Mes = MONTH(@FechaVer) 
...
-- Description:	GUARDAR VIAJE ESTADO
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Previajes_GuardarViajeEstado]
@idticket INT,
@Estado INT,
@Codviaje VARCHAR(80),
...
BEGIN TRAN
BEGIN TRY
	IF @Estado = 10 AND @UserCrea NOT IN ('RCORZO') BEGIN						
		IF EXISTS(SELECT * FROM ReportesApp_Combustible_TicketsSurtidor WHERE
		NroTicketPreViaje = CAST(RIGHT(@anio, 2) AS VARCHAR(02)) + CAST(@idticket AS VARCHAR(20))) BEGIN
			SET @Exito = '-2 = No se puede Anular, ya se le abasteció a la unidad con este Codigo de Ticket: ' 
								+ CAST(RIGHT(@anio, 2) AS VARCHAR(02)) + CAST(@idticket AS VARCHAR(20)) 
								+ '. Comunicarse con la Sra. Marisol García.'
			ROLLBACK
			GOTO Terminar
		END												
	END
						
	IF (SELECT ISNULL(Impreso,0)FROM ReportesApp_Operacion_Previaje_Registros 
	WHERE IdProgramacion = @idticket AND Anio = @anio)>0 AND @Estado = 10 BEGIN
		SET @Exito = '-4 = No se puede Anular la programacion tiene ticket impreso'												
						   + ' solicitar el ticket al Conductor y comunicarse con la Sra. Marisol García.'
		ROLLBACK
		GOTO Terminar
	END	

	IF @UserCrea NOT IN('AAVALOS') AND @Estado = 10 BEGIN
		SET @exito = '-1 = No tiene permiso para Anular programaciones comunicarse con  la Sra. Marisol García.'
		ROLLBACK
		GOTO TERMINAR
	END
	
	UPDATE ReportesApp_Operacion_Previaje_Registros
	SET CodViaje = @Codviaje, Estado = @Estado, Comentario = @Comentario
	WHERE IdProgramacion = @idticket AND Anio = @anio
	
	INSERT INTO ReportesApp_Operacion_Previajes_HistoricoProgramEstados (idprogramacion,fecha,ESTADO,observacion,fechausuario,ubicacionunidad,userregistra,fecharegistra)
	VALUES(@idticket,GETDATE(),@Estado,'',GETDATE(),'',@UserCrea,GETDATE())
	...
	DECLARE @Programacion INT = (SELECT TipoProgramacion FROM ReportesApp_Operacion_Previaje_Registros WHERE NroTicket = @NroTicket)
	DECLARE @idTracto INT = (SELECT idTracto FROM ReportesApp_Operacion_Previaje_Registros WHERE NroTicket = @NroTicket)
	DECLARE @FechaInicio DATE = (SELECT CONVERT(DATE,FechaInicio) FROM ReportesApp_Operacion_Previaje_Registros WHERE NroTicket = @NroTicket)
	DECLARE @FechaFin DATE = (SELECT CONVERT(DATE,FechaFin) FROM ReportesApp_Operacion_Previaje_Registros WHERE NroTicket = @NroTicket)

	IF (@Estado IN (1,9) AND @Programacion IN (1,2,3,4) AND EXISTS(SELECT * FROM ReportesApp_Operaciones_Operatividad_PlacasView WHERE IdUnidad = @idTracto
	AND Mes = MONTH(@FechaInicio) AND Anio = YEAR(@FechaInicio))) BEGIN
		WHILE (@FechaInicio <= @FechaFin) BEGIN
			DECLARE @co2 INT
			DECLARE @idCondicion INT = (SELECT CASE WHEN @Programacion = 1 THEN 3 WHEN @Programacion = 2 THEN 1 WHEN @Programacion = 3 THEN 2 ELSE 4 END)
			DECLARE @CondicionAnterior INT = (SELECT idCondicion FROM ReportesApp_Operaciones_Operatividad_Registro WHERE IdUnidad = @idTracto AND Fecha = @FechaInicio)
			...
			INSERT INTO ReportesApp_Operaciones_Operatividad_Registro(idOperatividad,IdUnidad,Fecha,idCondicion,IdOperacion,UsuarioRegistra,FechaRegistra)
			VALUES(@co2, @idTracto, @FechaInicio, @idCondicion, @Programacion, @UserCrea, GETDATE())
			...
			DECLARE @CantidadCondiciones INT = (SELECT COUNT(*) FROM ReportesApp_Operaciones_Operatividad_Registro
												WHERE Fecha = @x_FechaSelec AND idCondicion = @idCondicion)
			
			IF (EXISTS(SELECT * FROM ReportesApp_Operaciones_Operatividad_PlacasView WHERE Anio = YEAR(@x_FechaSelec) AND Mes = MONTH(@x_FechaSelec)
			...
			IF (EXISTS(SELECT * FROM ReportesApp_Operaciones_Operatividad_View WHERE Anio = YEAR(@x_FechaSelec) AND Mes = MONTH(@x_FechaSelec))) BEGIN
				UPDATE T 
...
-- Description:	GUARDAR VIAJE ESTADO
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Previajes_ActualizarViajeEstado]
@IDTicket INT,
@Anio SMALLINT,
@IDTracto INT,
@Estado INT,
...
BEGIN TRY
	...
	IF (@Estado IN (1,9) AND @TipoProgramacion IN (1,2,3,4,10) AND EXISTS(SELECT * FROM ReportesApp_Operaciones_Operatividad_PlacasView WHERE IdUnidad = @IDTracto
	AND Mes = MONTH(@FechaInicio) AND Anio = YEAR(@FechaInicio))) BEGIN
		WHILE (@FechaInicio <= @FechaFin) BEGIN
			...
			SET @correlativo = (SELECT MAX(idOperatividad) FROM ReportesApp_Operaciones_Operatividad_Registro)
			SET @correlativo = ISNULL(@correlativo,0) + 1
...
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Operatividad_ActualizarViajeTolvas]
@NroTicket INT,
@IDTracto INT,
@FechaInicio DATE,
@Usuario VARCHAR(80)
...
BEGIN TRAN
BEGIN TRY
	IF (EXISTS(SELECT * FROM ReportesApp_Operaciones_Operatividad_PlacasView WHERE IdUnidad = @IDTracto AND Mes = MONTH(@FechaInicio) AND Anio = YEAR(@FechaInicio))) BEGIN
		DECLARE @correlativo INT
		...
		DECLARE @x_DiaSelec INT = (SELECT DAY(@FechaInicio))
		DECLARE @Condicion VARCHAR(50) = (SELECT C.Codigo FROM ReportesApp_Operaciones_Operatividad_Condiciones C WHERE C.idCondicion = @idCondicion)
		DECLARE @CantidadCondiciones INT = (SELECT COUNT(*) FROM ReportesApp_Operaciones_Operatividad_Registro WHERE Fecha = @x_FechaSelec AND idCondicion = @idCondicion)

		IF (EXISTS(SELECT * FROM ReportesApp_Operaciones_Operatividad_PlacasView WHERE Anio = YEAR(@x_FechaSelec) AND Mes = MONTH(@x_FechaSelec) AND IdUnidad = @IDTracto)) BEGIN
			UPDATE P 
			SET P.D1 = CASE WHEN DAY(@x_FechaSelec) = 1 THEN @Condicion ELSE P.D1 END,
				P.D2 = CASE WHEN DAY(@x_FechaSelec) = 2 THEN @Condicion ELSE P.D2 END,
				P.D3 = CASE WHEN DAY(@x_FechaSelec) = 3 THEN @Condicion ELSE P.D3 END,
...
-- Author:		GERARDO REYES
-- Create date: 26-08-2024
-- Description:	GUARDAR VIAJE ESTADO - OTROS ESTADOS
-- =============================================
ALTER PROCEDURE [dbo].[ReportesApp_Operaciones_Operatividad_ActualizarViajeOtros]
@idCondicion INT,
@IDTracto INT,
@FechaInicio DATE,
@Usuario VARCHAR(80)
